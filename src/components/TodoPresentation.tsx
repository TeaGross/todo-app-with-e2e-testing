...
export const TodoPresentation = ({todo, onToggleDone, onDelete}: TodoProps) => {
    ...
    return <>
        <li  className={`flex items-center justify-center h-20`}>
                    ...
                    <span className={`w-[60%] md:text-[1.5rem]} ${todo.isDone ? "text-gray-1000 line-through opacity-60" : ""}`}>{todo.title}</span>
                    <button className="bg-[#231942] border-none rounded w-[60px] h-[40px] mr-2 md:w-[80px] disabled:bg-gray-600 disabled:cursor-not-allowed" onClick={() => onDelete(todo.id, todo.isDone)} disabled = {!todo.isDone} >🗑️</button>
                </li>
    </>
}